...
- **[`discrete-probability`](tools/discrete-probability/)** — an MCP server for exact discrete-probability calculations (birthday-paradox collisions, dice-sum distributions, drawing without replacement, binomial trials, Bayes' theorem, a generalized Monty Hall problem) plus `compare_two_proportions` (a two-proportion z-test with Wilson/Newcombe confidence intervals and a plain verdict, for "is this rate gap real or noise?"), each answer optionally cross-checked against a real CSPRNG-backed simulation — language models are well-documented to do fine on standard probability questions but drop sharply on "counterintuitive" ones.
...
- **[`graph-algorithms`](tools/graph-algorithms/)** — an MCP server for the classic graph algorithms (shortest path, topological sort, minimum spanning tree, max flow, graph coloring, maximum bipartite matching, the weighted assignment problem), each solved via one algorithm and independently checked via a second, structurally different one (e.g. Dijkstra solved, Bellman-Ford verified; graph coloring solved via DSATUR + forward-checking backtracking, its `chromatic_number` proving the minimum colors needed with a clique lower bound plus exhaustive uncolorability at every smaller count; the weighted assignment problem solved via the Hungarian algorithm and proven optimal via LP duality) — actively-benchmarked LLM failure territory as graphs grow past a handful of nodes, distinct from every other shape in this collection.
- **[`spaced-arrangement`](tools/spaced-arrangement/)** — an MCP server that constructs an ordering of items so no two items of the same category land within `min_distance` positions of each other (a playlist shuffle, a task queue, an interview schedule), and proves it via budgeted backtracking search with CSPRNG-randomized tie-breaking — a uniformly random shuffle of a category-heavy list clusters same-category items together far more than people expect, which is exactly why Spotify and Apple both rewrote their shuffle algorithms around this. `maximize_min_distance` finds the largest spacing achievable with no caller-chosen value needed, and proves it's the largest — the rigorous "spread as evenly as possible" mode.
