...
- **[`graph-algorithms`](tools/graph-algorithms/)** — an MCP server for the classic graph algorithms (shortest path, topological sort, minimum spanning tree, max flow, graph coloring), each solved via one algorithm and independently checked via a second, structurally different one (e.g. Dijkstra solved, Bellman-Ford verified; graph coloring solved via DSATUR + forward-checking backtracking, its `chromatic_number` proving the minimum colors needed with a clique lower bound plus exhaustive uncolorability at every smaller count) — actively-benchmarked LLM failure territory as graphs grow past a handful of nodes, distinct from every other shape in this collection.
